...
package com.sty.dj.barcodescaledemo.filechooser;
...
import com.sty.dj.barcodescaledemo.R;
						...
						if(!fileList[i].isDirectory()) {
							...
							if(this.filter!=null){
								if(this.filter.contains(";")){
									String[] list = this.filter.split(";");
									for(int j=0;j<list.length;j++){
										bMatch = fileList[i].getName().matches(list[j]);
										if(bMatch){
											break;
										}
									}
								}else{
									bMatch = fileList[i].getName().matches(this.filter);
								}
							}
							selectable = !this.folderMode && (this.filter == null || bMatch);
						} else {
